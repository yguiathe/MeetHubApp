...
static final Logger logger = LoggerFactory.getLogger(AccountController.class);
	...
	public String getAccountDetails(@PathVariable("actType") String actType, @SessionAttribute("membership") Membership membership, Model model) {
		logger.debug("Account Type: " + actType);
		Account act = accountService.findByMembershipAndAcctType(membership, actType);
		if (act != null) {
	...
	public String getTransactionForm(@PathVariable("actId") Long actId, @PathVariable("transType") String transType, @SessionAttribute("membership") Membership membership, Model model) {
		TransactionDto transDto = new TransactionDto(accountService.findById(actId).get(), new BigDecimal("0"), transType, "");
	...
	public String contribute(@ModelAttribute("transDto") @Validated TransactionDto transDto, BindingResult result, @SessionAttribute("user") User user, Model model) {
		if (result.hasErrors()) {
			logger.info("Binding Errors : {}", result.getAllErrors().get(0));
			return "/home";
		} else {
			accountService.contribute(transDto.getOriginatingAct(), transDto.getMtgCheckingAct(), transDto.getAmount(), user.getFnLN(), transDto.getNotes());
		}
		return "fragments/account::successful-transaction";
	}
	
	@RequestMapping(value = "/save", method = RequestMethod.POST)
	public String saving(@ModelAttribute("transDto") @Validated TransactionDto transDto, BindingResult result, @SessionAttribute("user") User user, Model model) {
		if (result.hasErrors()) {
			logger.info("Binding Errors : {}", result.getAllErrors().get(0));
			return "/home";
		} else {
			accountService.addToSavings(transDto.getOriginatingAct(), transDto.getMtgSavingAct(), transDto.getAmount(), user.getFnLN(), transDto.getNotes());
		}
		return "fragments/account::successful-transaction";
	}
	
	@RequestMapping(value = "/deposit", method = RequestMethod.POST)
	public String deposit(@ModelAttribute("transDto") @Validated TransactionDto transDto, BindingResult result, @SessionAttribute("user") User user, Model model) {
		if (result.hasErrors()) {
			logger.info("Binding Errors : {}", result.getAllErrors().get(0));
			return "/home";
		} else {
	...
	public String withdraw(@ModelAttribute("transDto") @Validated TransactionDto transDto, BindingResult result, @SessionAttribute("user") User user, Model model) {
		if (result.hasErrors()) {
			logger.info("Binding Errors : {}", result.getAllErrors().get(0));
			return "/home";
		} else {
